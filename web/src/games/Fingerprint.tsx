...
function FingerprintGraphic({
  ...
  return (
    <svg
      className="fingerprint__graphic"
      viewBox="0 0 200 240"
      style={{
        top: `${-index * bandHeight}px`,
        transform: `translateX(calc(-50% + ${offset * 22}px))`,
      }}
      aria-hidden="true"
    >
      <g>
        {RIDGES.map((path, ridge) => (
          <path key={ridge} d={path} />
        ))}
      </g>
    </svg>
  );
...
export function Fingerprint({ options, onComplete }: GameProps) {
  ...
  const [offsets, setOffsets] = useState(() =>
    Array.from({ length: slices }, () => 1 + Math.floor(Math.random() * 4)),
  );
  const [scanning, setScanning] = useState(false);
  const aligned = offsets.filter((value) => value === 0).length;
  const shift = (index: number, direction: number) =>
    setOffsets((values) =>
      values.map((value, slice) =>
        slice === index ? (value + direction + 5) % 5 : value,
      ),
    );
  const scan = () => {
    setScanning(true);
    ...
    >
      <div className={`fingerprint${scanning ? " is-scanning" : ""}`}>
        <div className="fingerprint__scanner">
          <i className="fingerprint__beam" />
          <div className="fingerprint__print">
            {offsets.map((offset, index) => (
              <div className="fingerprint__slice" key={index}>
                <button
                  onClick={() => shift(index, -1)}
                  aria-label={`Move fingerprint slice ${index + 1} left`}
                >
                  <svg viewBox="0 0 20 20">
                    <path d="m13 4-6 6 6 6" />
                  </svg>
                </button>
                <span className={offset === 0 ? "is-aligned" : ""}>
                  <FingerprintGraphic
                    index={index}
                    slices={slices}
                    offset={offset}
                ...
                <button
                  onClick={() => shift(index, 1)}
                  aria-label={`Move fingerprint slice ${index + 1} right`}
                >
                  <svg viewBox="0 0 20 20">
                    <path d="m7 4 6 6-6 6" />
                  </svg>
