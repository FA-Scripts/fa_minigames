...
export function Terminal({ options, onComplete }: GameProps) {
  ...
  const refreshInterval = clampOption(options, 'refreshInterval', 10000, 1000, 60000)
  ...
  const [clock, setClock] = useState({ remaining: timeLimit, refresh: refreshInterval })
  const deadline = useRef(Date.now() + timeLimit)
  const nextRefresh = useRef(Date.now() + refreshInterval)
  const finished = useRef(false)
  const serials = useMemo(() => Array.from({ length: count }, (_, index) => `0x${(4096 + index * 16).toString(16)}`), [count])
  ...
  const select = (entry: string) => {
    if (entry !== target || finished.current) return onComplete(false, { roundsCompleted: round - 1 })
    if (round >= rounds) {
      finished.current = true
  ...
  return <GamePanel eyebrow="PACKET SNIFFER // FA-40" title="Trace the access token" status={`TRACE ${round}/${rounds}`} instructions="Find the requested token before the table refreshes or the session expires.">
    <div className="terminal">
      <div className="terminal__target"><span>WATCH TOKEN</span><strong>{target}</strong><div><b>TIME {String(Math.floor(seconds / 60)).padStart(2, '0')}:{String(seconds % 60).padStart(2, '0')}</b><b>REFRESH {(clock.refresh / 1000).toFixed(1)}s</b></div><i /></div>
      <div className="terminal-grid">{entries.map((entry, index) => <button key={`${serials[index]}-${entry}`} onClick={() => select(entry)}><span>{serials[index]}</span><b>{entry}</b><i style={{ animationDuration: `${refreshInterval}ms` }} /></button>)}</div>
    </div>
  </GamePanel>
}
